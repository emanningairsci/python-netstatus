...
class readDB():
    ...
    #| The main collection used within that database is 'log'
    collection = db.log
    ...
    #| TODO: Currently, nothing is being done with those write results. It'd be nice to see them in an alert on the 
    #|       web page after a host is added. Same for delete_host and edit_host below.
    #|---------------------------------------------------------------------------------------------------------------
    def add_host(self, new_hostname, new_ip):
        client = MongoClient('localhost', 27017)
        db = client.netstatusDB
        write_result_log = db.log.update({"hostname": new_hostname}, {'$set': {"hostname": new_hostname, "IP": new_ip, "status": "Unknown", "uptime": 0, "downtime": 0} }, True )
    ...
    def delete_host(self, hostname):
        client = MongoClient('localhost', 27017)
        db = client.netstatusDB
        write_result_log = db.log.remove({"hostname": hostname})
    ...
    def edit_host(self, hostname, new_hostname, new_ip, service, serv_type, remove_service):
        client = MongoClient('localhost', 27017)
        db = client.netstatusDB
        write_result = db.log.update({"hostname": hostname}, {'$set': {"hostname": new_hostname, "IP": new_ip} }, True )
        ...
        #| remove the service if requested
    ...
    #|---------------------------------------------------------------------------------------
    #| get_unknown_hosts: like online_hosts, returns a matrix of hosts that are currently  
    #| logged as 'Unknown' status in MongoDB.
    #|---------------------------------------------------------------------------------------
    def get_unknown_hosts(self):
        unknown_hosts = []
        host_data = []
        for host in self.collection.find({"status":"Unknown"},{"hostname":1,"downtime":1, "timestamp":1, "IP":1, "_id":0}):
            host_data = [ str(host["hostname"]), str(host["timestamp"]), str(host["uptime"]), str(host["IP"]) ]
            unknown_hosts.append(host_data)
